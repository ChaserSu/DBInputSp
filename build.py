...
# ===================== ANSI颜色/加粗控制码（核心修正COLOR_RED_BOLD） =====================
# 样式说明：\033[1m 加粗 | \033[31m 红色 | \033[32m 绿色 | \033[34m 蓝色 | \033[0m 重置样式
COLOR_BLUE_BOLD = "\033[1;34m"   # 蓝色加粗（用户输入的中文）
COLOR_RED_BOLD = "\033[1;31m"    # 红色加粗
COLOR_GREEN_BOLD = "\033[1;32m"  # 绿色加粗（双拼编码/对勾）
COLOR_YELLOW_BOLD = "\033[1;33m" # 黄色加粗（声母）
COLOR_ORANGE_BOLD = "\033[1;38;5;208m" # 橙色加粗（韵母，使用256-color ANSI code for orange）
...
DEFAULT_SCHEME_NUM = 1  # 默认方案编号（从config读取）
HISTORY_ENABLE = 0  # 历史记录开关（0/1）
CLRHIS_LINE_NUM = 0  # 历史记录清除阈值行数
HISTORY_FILE_PATH = ""  # 历史记录文件路径
AUTOUPDATE_ENABLE = 0  # 自动检查更新开关（0/1）
SHOW_DETAIL = 0  # 展示详情开关（0=不展示过滤前结果，1=展示）
TOOL_VERSION = "v0.0.19"  # 工具版本号（从config读取）
...
def print_usage_guide():
    ...
    print(f"{COLOR_BOLD}【双拼转换】{COLOR_RESET}")  # 【输入内容】替换为【双拼转换】
    ...
    print(f"⌨️ {COLOR_YELLOW_BOLD}输入按键回车{COLOR_RESET} → {COLOR_ORANGE_BOLD}查询键位{COLOR_RESET}")
    print("🔀 混合输入回车 → 分行处理")
    print(f"{COLOR_BOLD}【切换方案】{COLOR_RESET}")
    print(f"📋 输入“@”回车 → 显示可用方案")
    print(f"🎯 输入“@方案名”回车，例如“@{default_scheme_name}”回车 → 切换对应方案")  # XXX替换为默认方案名
    ...
    print(f"⏮️ 输入“/”回车 → 切换首个方案：（首个：→ {first_num} {first_scheme_name}）")  # emoji与文字间一个空格
    print(f"🔽 输入“+”回车 → 切换下个方案：（循环：→ {next_num} {next_scheme_name}）")
    print(f"🔼 输入“-”回车 → 切换上一个方案（循环：→ {prev_num} {prev_scheme_name}）")
    print(f"⏭️ 输入“\\”回车 → 切换末个方案：（末个：→ {last_num} {last_scheme_name}）")  # emoji与文字间一个空格
    print(f"⭐ 输入“*”回车 → 切换默认方案：（默认：→ {default_num} {default_scheme_name}）")  # emoji与文字间一个空格
    print(f"{COLOR_BOLD}【其他操作】{COLOR_RESET}")
    print("🧹 输入“.”或“。”回车 → 清空屏幕")
    print(f"🕒 输入“%”回车 → 开/关历史（当前{history_status}）")  # 增加历史状态备注
    print(f"🗑️ 输入“>”或“》”回车 → 手动清空历史（当前history.txt有{history_line_count}条，达到{clrhis_value}条后自动清空）")  # 增加历史文件行数和clrhis备注
    ...
    print(f"📟 python main.py \"豆包\" → 直接调用")
    print(f"📟 DBInputSp.exe \"豆包\" → 直接调用")
    
    print(f"{COLOR_BOLD}【帮助与退出】{COLOR_RESET}")
    print(f"❓ 输入“?”或“？”回车 → 显示本指南")
    print("🚶 Ctrl+C → 退出程序")
    # 核心在使用指南最后一行后添加空行（分隔输入提示符）
    print()
    # 写入历史记录（非编码相关，会被过滤）
...
# ===================== 读取config.py中的配置项 =====================
def load_config():
    """读取config.py中的方案列表、default、history、clrhis、version、autoupdate、showdetail配置"""
    scheme_dict = {}
    default_num = 1
    history = 0
    clrhis = 0
    ...
    try:
        ...
        # 解析showdetail配置
        showdetail_match = re.search(r'showdetail\s*=\s*(\d+)', config_content)
        if showdetail_match:
            showdetail = int(showdetail_match.group(1))
            showdetail = 1 if showdetail == 1 else 0
        ...
        return scheme_dict, default_num, history, clrhis, version, autoupdate, showdetail
...
def reverse_convert_single(code_str, shengmu_map, yunmu_map, ling_shengmu_map, reverse_map):
    """
    反查单个双拼编码 → 全拼（oo/aa/ee整串解析+结果列表追加+v→ü/ui完整映射+多结果输出）
    ...
    for code in code_list:
        ...
        # 单字符非零声母（兜底）
            ...
            if code_item in ym_key_to_py:
                yunmu_mappings.extend(ym_key_to_py[code_item])
                possible_pys.extend(ym_key_to_py[code_item])
            # 尝试作为声母
            if code_item in sm_key_to_py:
                shengmu_mappings.append(sm_key_to_py[code_item])
    ...
    if not unique_combinations or not unique_combinations[0]:
        ...
        # 有过滤掉的结果，根据showdetail配置决定是否显示过滤前的结果
        filtered_results = "/".join(filtered_combinations) if filtered_combinations else "/".join(unique_combinations)
        if SHOW_DETAIL == 1:
            # 展示过滤前的结果
            ...
            # 不展示过滤前的结果，只显示过滤后的结果
            final_quangpin = filtered_results
...
def auto_run(input_content):
        ...
        if arg.startswith("=") and arg[1:].isdigit():
            ...
            if scheme_num in SCHEME_LIST:
                ...
                if save_config(scheme_num):
                    ...
                    SCHEME_LIST, DEFAULT_SCHEME_NUM, HISTORY_ENABLE, CLRHIS_LINE_NUM, TOOL_VERSION, AUTOUPDATE_ENABLE, SHOW_DETAIL = load_config()
                    # 切换到该方案
    ...
    if input_content.strip() in ['!', '！']:
        ...
        print()
        print("📋 切换方案快捷操作：")
        print(f"⏮️ 输入“/”回车 → 切换首个方案：（首个：→ {first_scheme}）")
        print(f"🔽 输入“+”回车 → 切换下个方案：（循环：→ {next_scheme}）")
        print(f"🔼 输入“-”回车 → 切换上个方案：（循环：→ {prev_scheme}）")
        print(f"⏭️ 输入“\\”回车 → 切换末个方案：（末个：→ {last_scheme}）")
        print(f"⭐ 输入“*”回车 → 切换默认方案：（默认：→ {default_scheme}）")
        print()  # 单行空行
    ...
    for seg in input_segments:
        if is_chinese(seg):
            ...
            # 检查是否是单个字符且可能同时对应声母和韵母
            if len(seg) == 1 and len(quangpin_result.split('/')) > 1:
                # 构建声母和韵母映射表用于查找
                sm_key_to_py = {v.lower(): k for k, v in shengmu.items()}
                ym_key_to_py = {}
                for py, key in yunmu.items():
                    key_lower = key.lower()
                    if key_lower not in ym_key_to_py:
                        ym_key_to_py[key_lower] = []
                    ym_key_to_py[key_lower].append(py)
                ...
                shengmu_mappings = []
                yunmu_mappings = []
                all_mappings = quangpin_result.split('/')
                
                for mapping in all_mappings:
                    if mapping in sm_key_to_py.values():
                        shengmu_mappings.append(mapping)
                    else:
                        yunmu_mappings.append(mapping)
                
                # 如果同时有声母和韵母映射，使用新格式
                if shengmu_mappings and yunmu_mappings:
                    shengmu_str = '/'.join(shengmu_mappings)
                    yunmu_str = '/'.join(yunmu_mappings)
                    reverse_msg = f"🔎 {COLOR_GREEN_BOLD}{split_code}{COLOR_RESET}【{CURRENT_SCHEME_NAME} → 全拼】【声母】{COLOR_YELLOW_BOLD}{shengmu_str}{COLOR_RESET} 【韵母】{COLOR_ORANGE_BOLD}{yunmu_str}{COLOR_RESET}"
                else:
                    # 否则使用原格式
...
def main_loop(file_path=None, input_content=None):
    ...
    # 初始化配置
    global SCHEME_LIST, DEFAULT_SCHEME_NUM, HISTORY_ENABLE, CLRHIS_LINE_NUM, AUTOUPDATE_ENABLE, TOOL_VERSION, SHOW_DETAIL
    SCHEME_LIST, DEFAULT_SCHEME_NUM, HISTORY_ENABLE, CLRHIS_LINE_NUM, TOOL_VERSION, AUTOUPDATE_ENABLE, SHOW_DETAIL = load_config()
    ...
    while True:
        try:
            # 输入提示符修改为 [方案编号 方案名] > 且保持加粗
            current_num = get_current_scheme_num()
            user_input = input(f"{COLOR_BOLD}[{current_num} {CURRENT_SCHEME_NAME}] >{COLOR_RESET} ")
            # 执行自动处理逻辑
            auto_run(user_input)
        except Exception as e:
            err_msg = f"❌ 程序运行出错：{e}"
